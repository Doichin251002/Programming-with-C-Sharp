...
namespace MovieStore.Tests.MovieTests
{
    public class MovieServiceTests
    {
        ...
        [Fact]
        public void Add_ShouldAddMovie_WhenActorExist()
        {
            var movie = new Movie { Id = "1", Title = "New Movie", Actors = new List<string> { "Actor1" } };

            _actorRepositoryMock.Setup(repo => repo.GetAllActors()).Returns(new List<Actor> { new Actor { Id = "Actor1" }});
        ...
        public void Add_ShouldThrowException_WhenActorDoesNotExist()
        {
            ...
            var exception = Assert.Throws<KeyNotFoundException>(() => _movieService.Add(movie));

            Assert.Equal("Actor with current ID does not exist", exception.Message);
        }
        ...
        public void Delete_ShouldThrowException_WhenMovieDoesNotExist()
        {
            ...
            _movieRepositoryMock.Setup(repo => repo.GetMovieById(id)).Returns((Movie)null);

            var exception = Assert.Throws<KeyNotFoundException>(() => _movieService.Delete(id));

            Assert.Equal($"Movie with ID {id} does not exist", exception.Message);
        }

        [Fact]
        public void Update_ShouldUpdateMovie_WhenMovieExists()
        {
            var movie = new Movie { Id = "1", Title = "Updated Movie", Year = 2000, Actors = new List<string> { "Actor1" }};
