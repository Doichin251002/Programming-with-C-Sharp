...
namespace MovieStore.Tests.ActorTests
{
    public class ActorServiceTests
    {
        ...
        public void Add_ShouldAddActor()
        {
            ...
            _actorRepositoryMock.Setup(repo => repo.GetAllActors()).Returns(new List<Actor> { });
        ...
        [Fact]
        public void Add_ShouldThrowException_WhenActorNameIsNull()
        {
            var actor = new Actor { Id = "1", Name = null };

            var exception = Assert.Throws<ArgumentException>(() => _actorService.Add(actor));

            Assert.Equal("Actor name cannot be null or empty. (Parameter 'Name')", exception.Message);
        }

        [Fact]
        public void Add_ShouldThrowException_WhenActorNameIsEmpty()
        {
            var actor = new Actor { Id = "1", Name = "" };
        ...
        {
            var actor = new Actor { Id = "1", Name = "John Doe" };
            _actorRepositoryMock.Setup(repo => repo.GetAllActors()).Returns(new List<Actor> { actor });

            var exception = Assert.Throws<InvalidOperationException>(() => _actorService.Add(actor));

            Assert.Equal($"Actor with name {actor.Name} already exists.", exception.Message);
        }
        ...
        [Fact]
        public void Update_ShouldThrowException_WhenActorIdIsNull()
        {
            var actor = new Actor { Id = null, Name = "John Doe" };

            var exception = Assert.Throws<ArgumentException>(() => _actorService.Update(actor));

            Assert.Equal("Actor ID cannot be null or empty. (Parameter 'Id')", exception.Message);
        }

        [Fact]
        public void Update_ShouldThrowException_WhenActorIdIsEmpty()
        {
            var actor = new Actor { Id = "", Name = "John Doe" };
        ...
        [Fact]
        public void Update_ShouldThrowException_WhenActorNameIsNull()
        {
            var actor = new Actor { Id = "1", Name = null };
        ...
        [Fact]
        public void Update_ShouldThrowException_WhenActorNameIsEmpty()
        {
            var actor = new Actor { Id = "1", Name = "" };
        ...
        [Fact]
        public void Delete_ShouldThrowException_WhenActorIdIsNull()
        {
            var exception = Assert.Throws<ArgumentException>(() => _actorService.Delete(null));

            Assert.Equal("Actor ID cannot be null or empty. (Parameter 'id')", exception.Message);
        }

        [Fact]
        public void Delete_ShouldThrowException_WhenActorIdIsEmpty()
        {
            var exception = Assert.Throws<ArgumentException>(() => _actorService.Delete(""));
